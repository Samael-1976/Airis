# src/connectors/microsoft_onedrive.py
# Esecutore Autonomo per Microsoft OneDrive (v2.0 - BaseConnector Refactor)
...
try:
    ...
    print('{"status": "error", "message": "' + t("microsoft_onedrive.lib_error") + '"}')
    sys.exit(1)
...
def list_files(connector: BaseConnector, access_token, params: dict):
    max_results = params.get("max_results", 10)
    connector.log_debug(f"Richiesta lista file root (Max: {max_results})...")
    ...
    try:
        response = requests.get(
            ...
            headers=headers,
            params={"$top": max_results, "$select": "name,lastModifiedDateTime,size,file,folder"},
        )
        ...
        connector.log_debug(f"Elementi trovati: {len(items)}")

        if not items:
            return t("microsoft_onedrive.no_files")

        raw_data_list =[]
        for item in items:
            name = item.get("name", "Senza nome")
            is_folder = "folder" in item
            item_type = "CARTELLA" if is_folder else "FILE"
            date_str = item.get("lastModifiedDateTime", "")
            try:
                date_obj = datetime.fromisoformat(date_str.replace("Z", "+00:00"))
                date = date_obj.strftime("%d/%m/%Y %H:%M")
            except ValueError:
                date = date_str
            raw_data_list.append(f"NOME: {name} | TIPO: {item_type} | MODIFICATO: {date}")
        ...
        smart_summary = ask_local_llm(
            ...
            context_description=t("microsoft_onedrive.context_list"),
        ...
        connector.log_debug(f"Eccezione List Files: {e}")
        raise e
...
if __name__ == "__main__":
    connector = BaseConnector("Connettore per Microsoft OneDrive.")
