# src/connectors/microsoft_outlook.py
# Esecutore Autonomo per Microsoft Outlook/Office 365 (v2.0 - BaseConnector Refactor)
# LEGGE A0099: Invarianza strutturale garantita.

import sys
import os
from pathlib import Path
from datetime import datetime

SCRIPT_DIR = Path(__file__).parent.resolve()
PROJECT_ROOT = SCRIPT_DIR.parent.parent
sys.path.insert(0, str(PROJECT_ROOT))

from utils.translator import t
from src.connectors.base_connector import BaseConnector

try:
    import msal
    import requests
    from src.guardian import Guardian
    from agent_base import ask_local_llm
except ImportError:
    print('{"status": "error", "message": "' + t("microsoft_outlook.lib_error") + '"}')
    sys.exit(1)

GRAPH_API_ENDPOINT = "https://graph.microsoft.com/v1.0"
SCOPES =["User.Read", "Mail.Read", "Mail.Send"]

def authenticate(connector: BaseConnector):
    guardian = Guardian()
    creds_config = guardian.get_credentials("microsoft_api")
    if not creds_config:
        connector.log_debug("Configurazione 'microsoft_api' mancante.")
        raise ValueError(t("microsoft_excel.auth_error"))

    token_path = PROJECT_ROOT / creds_config.get("token_path", "config/microsoft_token.json")

    cache = msal.SerializableTokenCache()
    if token_path.exists():
        connector.log_debug("Cache token trovata, caricamento...")
        cache.deserialize(token_path.read_text())

    app = msal.PublicClientApplication(
        creds_config["client_id"],
        authority=creds_config["authority"],
        token_cache=cache,
    )

    accounts = app.get_accounts()
    result = None
    if accounts:
        connector.log_debug(f"Account trovato in cache: {accounts[0].get('username')}")
        result = app.acquire_token_silent(SCOPES, account=accounts[0])

    if not result:
        connector.log_debug("Nessun token valido in cache. Avvio Device Flow...")
        flow = app.initiate_device_flow(scopes=SCOPES)
        if "user_code" not in flow:
            raise ValueError("Fallimento nell'ottenere il codice utente.", flow.get("error_description"))

        print(t("microsoft_excel.device_flow_msg", uri=flow["verification_uri"], code=flow["user_code"]))
        sys.stderr.write(f"ATTESA AUTENTICAZIONE UTENTE (Codice: {flow['user_code']})...\n")

        result = app.acquire_token_by_device_flow(flow)

    if "access_token" in result:
        if cache.has_state_changed:
            token_path.write_text(cache.serialize())
            connector.log_debug("Token salvato su disco.")
        return result["access_token"]
    else:
        err_desc = result.get("error_description", "Errore sconosciuto")
        connector.log_debug(f"Errore autenticazione: {err_desc}")
        raise Exception(err_desc)

def list_messages(connector: BaseConnector, access_token, params: dict):
    max_results = params.get("max_results", 5)
    connector.log_debug(f"Richiesta messaggi Inbox (Max: {max_results})...")
    headers = {"Authorization": f"Bearer {access_token}"}

    try:
        response = requests.get(
            f"{GRAPH_API_ENDPOINT}/me/mailFolders/inbox/messages",
            headers=headers,
            params={
                "$top": max_results,
                "$select": "subject,from,receivedDateTime,bodyPreview",
                "$orderby": "receivedDateTime DESC",
            },
        )

        if response.status_code != 200:
            connector.log_debug(f"Errore API: {response.text}")

        response.raise_for_status()
        messages = response.json().get("value",[])
        connector.log_debug(f"Messaggi trovati: {len(messages)}")

        if not messages:
            return t("microsoft_outlook.no_messages")

        raw_data_list =[]
        for msg in messages:
            sender = msg.get("from", {}).get("emailAddress", {}).get("name", "Sconosciuto")
            subject = msg.get("subject", "Nessun Oggetto")
            preview = msg.get("bodyPreview", "")
            date_str = msg.get("receivedDateTime", "")
            try:
                date_obj = datetime.fromisoformat(date_str.replace("Z", "+00:00"))
                date = date_obj.strftime("%d/%m/%Y %H:%M")
            except ValueError:
                date = date_str
            raw_data_list.append(f"DATA: {date} | DA: {sender} | OGGETTO: {subject} | ANTEPRIMA: {preview}")

        raw_string = "\n---\n".join(raw_data_list)
        avatar_name = os.environ.get("AIRIS_ACTIVE_AVATAR", "L'Anima")
        connector.log_debug("Inoltro email all'agente locale per il Triage...")

        smart_summary = ask_local_llm(
            data_to_analyze=raw_string,
            context_description=t("microsoft_outlook.context_list"),
            avatar_name=avatar_name,
        )

        return smart_summary

    except Exception as e:
        connector.log_debug(f"Eccezione List Messages: {e}")
        raise e

def send_message(connector: BaseConnector, access_token, params: dict):
    required_params =["to", "subject", "body"]
    if not all(p in params for p in required_params):
        raise ValueError(f"Parametri mancanti per inviare l'email. Richiesti: {', '.join(required_params)}")

    connector.log_debug(f"Invio email a: {params['to']}, Oggetto: {params['subject']}")

    headers = {
        "Authorization": f"Bearer {access_token}",
        "Content-Type": "application/json",
    }
    email_data = {
        "message": {
            "subject": params["subject"],
            "body": {"contentType": "Text", "content": params["body"]},
            "toRecipients": [{"emailAddress": {"address": addr.strip()}} for addr in params["to"].split(",")],
        },
        "saveToSentItems": "true",
    }

    try:
        response = requests.post(f"{GRAPH_API_ENDPOINT}/me/sendMail", headers=headers, json=email_data)

        if response.status_code not in[200, 202]:
            connector.log_debug(f"Errore API Send: {response.text}")

        response.raise_for_status()
        connector.log_debug("Email inviata con successo.")

        avatar_name = os.environ.get("AIRIS_ACTIVE_AVATAR", "L'Anima")
        confirmation_data = t("microsoft_outlook.send_confirm", to=params["to"], subject=params["subject"])

        smart_confirmation = ask_local_llm(
            data_to_analyze=confirmation_data,
            context_description=t("microsoft_outlook.context_send"),
            avatar_name=avatar_name,
        )

        return smart_confirmation

    except Exception as e:
        connector.log_debug(f"Eccezione Send Message: {e}")
        raise e

def action_wrapper(connector: BaseConnector, action_func, params: dict):
    access_token = authenticate(connector)
    return action_func(connector, access_token, params)

if __name__ == "__main__":
    connector = BaseConnector("Connettore per Microsoft Outlook.")
    connector.register_action("list", lambda params: action_wrapper(connector, list_messages, params))
    connector.register_action("send", lambda params: action_wrapper(connector, send_message, params))
    connector.run()