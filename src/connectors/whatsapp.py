...
# Esecutore Autonomo per WhatsApp via Twilio (v2.0 - BaseConnector Refactor)
...
try:
    ...
    print('{"status": "error", "message": "' + t("whatsapp.lib_error") + '"}')
    sys.exit(1)

def send_whatsapp_message(connector: BaseConnector, params: dict, creds: dict):
    ...
    if not all([account_sid, auth_token, from_number]) or "IL_TUO" in account_sid:
        ...
        raise ValueError(t("whatsapp.auth_error"))
    ...
    try:
        ...
        to_whatsapp_number = f'whatsapp:{params["to_number"]}'
        from_whatsapp_number = f"whatsapp:{from_number}"

        connector.log_debug(f"Invio WhatsApp a {to_whatsapp_number} da {from_whatsapp_number}...")

        message = client.messages.create(
            to=to_whatsapp_number, from_=from_whatsapp_number, body=params["body"]
        )

        connector.log_debug(f"Messaggio inviato. SID: {message.sid}")

        avatar_name = os.environ.get("AIRIS_ACTIVE_AVATAR", "L'Anima")
        confirmation_data = t(
            "whatsapp.send_confirm",
            to=params["to_number"],
            body=params["body"],
            sid=message.sid,
        )
        ...
        smart_confirmation = ask_local_llm(
            ...
            context_description=t("whatsapp.context_send"),
        ...
        connector.log_debug(f"Errore invio WhatsApp: {e}")
        raise e
...
if __name__ == "__main__":
    connector = BaseConnector("Connettore per WhatsApp via Twilio.")
    connector.register_action("send_message", lambda params: action_wrapper(connector, send_whatsapp_message, params))
    connector.run()